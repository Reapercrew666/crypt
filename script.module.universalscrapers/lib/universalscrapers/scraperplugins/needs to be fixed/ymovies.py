...
class scrape_thread(threading.Thread):
    ...
    def run(self):
        try:
            qual = self.qual
            url = 'https://yesmovies.ag/ajax/movie_token?eid='+self.m+'&mid='+self.match
            html3 = requests.get(url).content
            x,y = re.findall("_x='(.+?)', _y='(.+?)'",html3)[0]
            fin_url = 'https://yesmovies.ag/ajax/movie_sources/'+self.m+'?x='+x+'&y='+y
...
class Yesmovies(Scraper):
    domains = ['yesmovies.ag']
    name = "yesmovies"

    def __init__(self):
        self.base_link = 'https://yesmovies.ag'
        ...
        try:
            ...
            for url,name in match:
                if clean_title(title)+'season'+season == clean_title(name):
                    ...
                    get_ep = requests.get('https://yesmovies.ag/ajax/v4_movie_episodes/'+match2).content
        ...
        try:
            ...
            for url,name in match:
                if clean_search(title.replace(' ','')) == clean_search(name).replace(' ',''):
                    ...
                    second_url = 'https://yesmovies.ag/ajax/v4_movie_episodes/'+match
