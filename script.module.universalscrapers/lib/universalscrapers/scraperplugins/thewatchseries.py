        ...
        try:
            ...
            for link in links:
                ...
                if 'vidnext.net' in link:
                    ...
                    count +=1
                    self.sources.append({'source': 'Mixdrop', 'quality': res, 'scraper': self.name, 'url': link,'direct': False})
                elif 'dood.to' in link:
                    get_res=requests.get(link).content
                    try:
                        res = re.compile('{type:"video/mp4".+?height:(.+?),',re.DOTALL).findall(get_res)[0]
                        count +=1
                        self.sources.append({'source': 'Dood', 'quality': res, 'scraper': self.name, 'url': link,'direct': False})
                    except:
                        pass
                else:
