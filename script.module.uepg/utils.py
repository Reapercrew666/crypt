...
#     Labels = {"title": label,"plot":plot, "duration":duration,
#         "StartTime":starttime, "ChannelName":channelname,
#         'genre':genre, 'fanart':fanart}
#     listitem = xbmcgui.ListItem()
#     listitem.setProperty("IsPlayable","true")
#     listitem.setProperty("IsInternetStream","true")
...
def getListItems(item):
    item       = dict(item)
    channelname = (item.get('channelname',''))
    plot = (item.get('plot','')) or ''
    art = (item.get('art',{})) or {}
    shthumb = (art.get('thumb', ''))
    shfanart = (art.get('fanart', ''))
    ...
    logo = (art.get('logo', ''))
    icon = (art.get('icon', ''))
    chthumb = (item.get('thumb',''))
    chfanart = (item.get('fanart',''))
    mediatype = (item.get('mediatype',''))
    genre = (item.get('genre',''))
    duration = (item.get('duration',''))
    starttime = (item.get('starttime',''))    
    ChanelData = {'shplot':plot, 'shthumb':shthumb, 'shfanart':shfanart,
                    'shlabel':label, 'chlink':link, 'chgenre':genre,
                    'shduration':duration, 'shstarttime':starttime,
                    'chchannelname':channelname, 'shmediatype':mediatype,
                    'shposter':post, 'shlogo':logo, 'shlabel2':label2,
                    'chthumb':chthumb, 'chfanart':chfanart, 'shicon':icon} 
    return ChanelData   
...
def contextListItem(item):
    ...
    #liz.setInfo( type="Video", infoLabels={"Title": label,"Plot":plot,})
