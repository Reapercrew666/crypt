...
class uEPG(xbmcgui.WindowXML):
    ...
    def setShowInfo(self):
        ...
        ChannelName     = self.getItemChannelName(newchan, plpos)
    ...
    def getContextItems(self, channel, position):
        position = self.fixPlaylistIndex(channel, position)
        item = self.channelLST.channels[channel - 1].guidedata[position]
        return item  
    ...
    def getItemChannelName(self, channel, position):
        position = self.fixPlaylistIndex(channel, position)
        item = self.channelLST.channels[channel - 1].guidedata[position]
        ChannelName = (item.get('channelname',''))
        utils.log('getItemChannelName, ChannelName = ' + ChannelName)
        return ChannelName  
    ...
    def test_list(self):
        ...
        newchan, chnoffset = self.getFocusChannel(newchan)
        plpos, reftime = self.determinePosAtTime(starttime, newchan)
        item = self.getContextItems(newchan, plpos)
        try: LI = utils.getListItems(item) 
        except: pass
        ChanName        = LI['chchannelname']
        ChanLink        = LI['chlink']
        ChanThumb       = LI['chthumb']
        ChanFanart      = LI['chfanart']
        ChanGenre       = LI['chgenre']
        ShowStarttime   = LI['shstarttime']
        ShowDuration    = LI['shduration']
        ShowPlot        = LI['shplot']
        ShowThumb       = LI['shthumb']
        ShowFanart      = LI['shfanart']
        ShowLabel       = LI['shlabel']
        ShowLabel2      = LI['shlabel2']
        ShowMediaType   = LI['shmediatype']
        ShowLogo        = LI['shlogo']
        ShowPoster      = LI['shposter']
        ShowIcon        = LI['shicon']
    ...
    def showInfo(self):
        ...
        plpos, reftime = self.determinePosAtTime(starttime, newchan)
        item = self.getContextItems(newchan, plpos)
        try: curListItem = utils.contextListItem(item)
        #try: curListItem = utils.buildListItem(item)
        except:
            pass
