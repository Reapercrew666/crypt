    ...
    def answer_request(self, sendData):
        ...
        try:
            ...
            if streamtype == 'HDS':
                ...
                if downloader.live is False and len(requested_range) > 0 and not requested_range == "bytes=0-0":  # we have to stream?
                    enableSeek = True
                ...
                if enableSeek:
                    ...
                    rtype = "flv-application/octet-stream"  #d efault type could have gone to the server to get it.
                    self.send_header("Content-Type", rtype)
                    self.send_header("Accept-Ranges", "bytes")
...
class f4mProxyHelper():
    ...
    def playF4mLink(self, url, name, proxy=None, use_proxy_for_chunks=False, maxbitrate=0, simpleDownloader=False, auth=None, streamtype='HDS', setResolved=False, swf=None, callbackpath="", callbackparam="", iconImage = "DefaultVideo.png"):
        try:
            ...
            try:
                if streamtype is None or streamtype == '' or streamtype in ['HDS'  'HLS', 'HLSRETRY']:
