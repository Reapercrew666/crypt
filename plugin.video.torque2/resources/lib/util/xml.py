...
wrapper class for jen list functions
    ...
    list_pattern = re.compile(
        '((?:<dogpiss>.+?</dogpiss>|<catpiss>.+?</catpiss>|<ratpiss>.+?</ratpiss>'
        '|<info>.+?</info>|'
    ...
    def __init__(self, url, cached=True):
        ...
        if xml:
            ...
            info = JenItem(xml.split('<dogpiss>')[0].split('<catpiss>')[0])
    ...
    def process_item(self, item_xml):
        ...
        if item.item_string.startswith("<dogpiss>"):
            title = item["title"]
            if title == "":
                title = item["name"]
            ...
            mode = "get_sources"
            link = item_xml
        elif item.item_string.startswith("<catpiss>"):
            title = item["name"]
            if title == "":
                title = item["title"]
            try:
            ...
            if item["link"].startswith("message"):
                ...
                mode = "get_list"
                link = item["link"]
        elif item.item_string.startswith("<ratpiss>"):
            link = item["link"]
            title = item["name"]
            if title == "":
                title = item["title"]
