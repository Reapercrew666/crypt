...
def ulib(string, enc=False):
    try:
        string = urllib.parse.quote(string) if enc else urllib.parse.unquote(string)
        return string
    except:
        return string
...
#         r = dom_parser2.parse_dom(c, 'div', {'class': re.compile(r'table-list-header-toggle\sstates\sfloat-left\spl-\d+')})
