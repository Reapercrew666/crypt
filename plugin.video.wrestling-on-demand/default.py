import xbmc, xbmcaddon, xbmcgui, xbmcplugin, urllib, urllib2, os, re, sys, datetime, shutil, random, liveresolver
from resources.libs.common_addon import Addon
import base64, resolveurl as urlresolver
from metahandler import metahandlers
from resources.libs import dom_parser, kodi

addon_id = 'plugin.video.wrestling-on-demand'
...
baseurl = 'http://L3grthu.tk/~hades/wod/base.xml'
...
messagetext = 'http://L3grthu.tk/~hades/wod/info.xml'
startinfo = 'http://L3grthu.tk/~hades/wod/startinfo.xml'
wweschedule = 'http://L3grthu.tk/~hades/wod/schedule.xml'
requests = 'http://L3grthu.tk/~hades/wod/requests.xml'
SEARCH_LIST = 'http://L3grthu.tk/~hades/wod/search.xml'
    ...
    for item in match:
        try:
            ...
            if '<sportsdevil>' in item:
                ...
                data = re.compile(
                    '<title>(.+?)</title>.+?folder>(.+?)</folder>.+?thumbnail>(.+?)</thumbnail>.+?fanart>(.+?)</fanart>').findall(
                    item)
                ...
                if len(links) == 1:
                    data = re.compile(
                        '<title>(.+?)</title>.+?link>(.+?)</link>.+?thumbnail>(.+?)</thumbnail>.+?fanart>(.+?)</fanart>').findall(
                        item)
                    lcount = len(match)
                    for name, url, iconimage, fanart in data:
            ...
            pass
        view(link)
    addLink('[B][COLOR mediumseagreen] ----------------     Version 6.02     ---------------- [/COLOR][/B]', url, 999,
            icon, fanarts)
...
def YOUTUBE_CHANNEL(url):
    ...
    for video in videos:
        ...
        item["url"] = base64.b64decode(
            b"cGx1Z2luOi8vcGx1Z2luLnZpZGVvLnlvdXR1YmUvcGxheS8/dmlkZW9faWQ9") + find_single_match(video,
                                                                                                 "<id>([^<]+)</id>")
        ...
        addLink('[COLOR white]' + item["name"] + ' - on ' + item["date"] + '[/COLOR]', item["url"], 7,
                item["iconimage"], fanart)
...
def DXTV_CATS(url):
    u = open_url(url)
    match = re.compile(
        '<article class=".+?">.+?<a href="(.+?)" title="(.+?)" rel="nofollow" id="featured-thumbnail">.+?<img.+?src="(.+?)".+?</article>',
        re.DOTALL).findall(u)
    for url2, name, image in match:
        name = name.replace('Watch', '').replace('Download', '').replace('online', '').replace('/', '').encode(
            'utf-8').lstrip()
        url2 = url2.encode('utf-8')
        iconimage = image.encode('utf-8')
        addLink(name, url2, 15, iconimage, fanart)
    np = re.compile(
        "<div class=\"pagination pagination-numeric\">.+?<li class='current'><span class='currenttext'>.+?</span></li><li><a rel='nofollow' href='(.+?)' class='inactive'>.+?</a>").findall(
        u)
    for nextpage in np:
...
def DXTV_LINKS(name, url):
    ...
    r = open_url(url)
    r = re.compile('<ifram.+?src="(.+?)"').findall(
        r)  # + re.compile('<span style="color: #008000;"><strong>(.+?)</strong>',re.DOTALL).findall(r)
    ...
    if len(r) == 0:
        ...
        if len(streamurl) == 1:
            PLAYLINK(name, streamurl[0], icon)
        else:
            dialog = xbmcgui.Dialog()
    ...
    if os.path.exists(REDDIT_FILE):
        ...
        if '<item>' in msg:
            ...
            for item in match:
                ...
                addDir('[COLOR snow]' + name.encode('utf-8') + '[/COLOR]', url.encode('utf-8'), 319, iconimage, fanart,
                       '', cm)
        else:
            addLink('No user added Reddits detected.', 'url', 999, iconimage, fanart)
    xbmc.executebuiltin("Dialog.Close(busydialog)")
...
def REDDIT_SUGGESTED():
    r = open_url('http://L3grthu.tk/~hades/wod/suggested.xml')
...
def REDDIT_GET(url):
    ...
    if not 'comments' in url:
        ...
        try:
            ...
            addLink('[COLORsnow][B]Welcome to the ' + sitename.title() + ' Reddit![/B][/COLOR]', 'url', 999, icon,
                    fanart)
        except:
            pass
        ...
        if r:
            for i in r:
                ...
                try:
                    score = re.compile('<div class="score unvoted" title=".+?">([0-9]+)</div>').findall(i.content)[0]
                except:
                    score = '0'
                try:
                    comments = re.compile('data-event-action="comments".+?>([0-9]+)\scomments<\/a>').findall(i.content)[
                        0]
                except:
                    comments = '0'
            ...
            for score, title, url, comments in combinedlist:
                title = title.replace('&amp;', '&')
                addDir(
                    '[COLORsnow][B]' + score + '[/B][/COLOR] - [COLOR white]' + title + '[/COLOR] - ' + comments + ' Comments',
                    url, 319, icon, fanarts)
        else:
            addLink('No Sub Reddits Found', 'url', 999, icon, fanart)
        ...
        u = [i[1] for i in r[1:] if
             ('<p>' in i[1]) and any(f for f in checks if f in i[1]) and not any(f for f in black if f in i[1])]
        ...
        if a:
            ...
            addLink('No Links Found', 'url', 999, icon, fanart)
        ...
        if combined:
            ...
            for count, name, url in tup:
                if count == '0':
                    if ace_got == 0:
                        addLink(
                            '[COLOR yellow][B]-===| Acestream Links | [I]Most Recent At Bottom[/I] |===-[/B][/COLOR]',
                            'url', 999, icon, fanart)
                        ace_got = 1
                    addLink(name.encode('utf-8'), url, 318, icon, fanart)
                else:
                    if http_got == 0:
                        addLink('[COLOR yellow][B]-===| Web Links | [I]Most Recent At Bottom[/I] |===-[/B][/COLOR]',
                                'url', 999, icon, fanart)
...
def EVENT_REDDIT():
    r = open_url('http://L3grthu.tk/~hades/wod/redditevent.xml')
    r = re.compile('<link>(.+?)</link>').findall(r)
        ...
        if combined:
            ...
            for count, name, url in tup:
                if count == '0':
                    if ace_got == 0:
                        addLink(
                            '[COLORyellow][B]-===| Acestream Links | [I]Most Recent At Bottom[/I] |===-[/B][/COLOR]',
                            'url', 999, icon, fanart)
                    ...
                    if http_got == 0:
                        addLink('[COLORyellow][B]-===| Web Links | [I]Most Recent At Bottom[/I] |===-[/B][/COLOR]',
                                'url', 999, icon, fanart)
...
def REDDIT_PLAYER(name, url, iconimage):
    ...
    if 'acestream' in url:
        ...
        url = 'plugin://plugin.video.SportsDevil/?mode=1&amp;item=catcher%3dstreams%26title=' + str(
            name) + '%26url=' + url + '%26referer=none'
    liz = xbmcgui.ListItem(name, iconImage=iconimage, thumbnailImage=iconimage)
    liz.setPath(url)
    dp.close()
    xbmc.Player().play(url, liz, False)
...
def open_url(url):
    try:
        ...
        link = response.read()
        response.close()
        link = link.replace('\n', '').replace('\r', '').replace('<fanart></fanart>', '<fanart>x</fanart>').replace(
            '<thumbnail></thumbnail>', '<thumbnail>x</thumbnail>').replace('<utube>',
                                                                           '<link>https://www.youtube.com/watch?v=').replace(
            '</utube>', '</link>')  # .replace('></','>x</')
        print link
        return link
    except:
        quit()
...
def addDir(name, url, mode, iconimage, fanart, description=''):
    u = sys.argv[0] + "?url=" + urllib.quote_plus(url) + "&mode=" + str(mode) + "&name=" + urllib.quote_plus(
        name) + "&description=" + str(description) + "&fanart=" + urllib.quote_plus(fanart)
    ok = True
    liz = xbmcgui.ListItem(name, iconImage="DefaultFolder.png", thumbnailImage=iconimage)
...
if mode == None or url == None or len(url) < 1:
    ...
    GETMULTI_SD(name, url, iconimage)
elif mode == 9:
    SHOW_PICTURE(url)
elif mode == 10:
    NEW()
    ...
    YOUTUBE_PLAYLIST_PLAY(url)
elif mode == 71:
    YOUTUBE_CHANNEL(url)
elif mode == 72:
    YOUTUBE_CHANNEL_PART2(url)
elif mode == 317:
    EVENT_REDDIT()
elif mode == 318:
    REDDIT_PLAYER(name, url, iconimage)
elif mode == 319:
    REDDIT_GET(url)
elif mode == 320:
    REDDIT_MAIN()
