        ...
        if user == "user-name":
            ...
            for res in html:   
                ...
                try:
                    ...
                    for results in  res:
                        if media == 'movie':
                            ...
                            if get_trailer == 'true':
                                ...
                                r = html['results'][0]
                                site = html['results'][0]['site']
                                if site == "YouTube":                                   
                                    trail_key = html['results'][0]['key']
                                else:
                                    trail_key = ""    
                        elif media == 'tv':
                            icon = results['poster_path']
                    ...
                    icon = "none"
                    fanart = "none"
                print_movie_xml(list_name, media, name, year, imdb, tmdb, icon, fanart, folder_name,trail_key)
...
def print_movie_xml(list_name, media, name, year, imdb, tmdb, icon, fanart, folder_name,trail_key):
    try:       
        if media == "movie":
            name = remove_non_ascii(name)
            xml_folder = os.path.join(xml_path,folder_name)
            ...
            f.write('\t<link>\n')
            if trail_key != "":
                trail_key = "https://www.youtube.com/watch?v=%s&feature=youtu.be (Play Trailer)" % (trail_key)
                f.write('\t<sublink>%s</sublink>\n' % trail_key)
            f.write('\t<sublink> (Play Movie Link 1)</sublink>\n')
            f.write('\t<sublink> (Play Movie Link 2)</sublink>\n')
            f.write('\t<sublink> (Play Movie Link 3)</sublink>\n')
            f.write('\t<sublink> (Play Movie Link 4)</sublink>\n')
            f.write('\t<sublink> (Play Movie Link 5)</sublink>\n')
            f.write('\t<sublink> (Play Movie Link 6)</sublink>\n')
            f.write('\t<sublink> (Play Movie Link 7)</sublink>\n')			
            f.write('\t<sublink>%s</sublink>\n' % "search")
            f.write('\t<sublink>%s</sublink>\n' % "searchsd")
            f.write('\t</link>\n')
