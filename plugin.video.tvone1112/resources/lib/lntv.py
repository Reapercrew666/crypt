...
class User(BaseModel):
    ...
    apk_name = TextField(default="com.core.lntmobile")
    apk_cert = TextField(default="B0:FD:D6:4B:79:29:3B:9B:3E:1E:C7:BE:4F:F4:BF:3B:8B:31:6C:04")
    apk_version = TextField(default="4.7.4 (41)")
    apk_build = TextField(default="41")
...
class LnTv(object):
    def __init__(self, cache_dir, cert, cert_key):
        ...
        self.api_key = None
        self.rapi_key = None
        self.api_url = "https://echo.livenettv.io/data/3/"
        ...
        DB = os.path.join(cache_dir, "lntv0.db")
        db.init(DB)
        db.connect()
        db.create_tables(
    ...
    def fetch_config(self):
        ...
        r = self.s.post(
            url, data=remoting.encode(request_form).getvalue(), headers={"Content-Type": "application/x-amf"}
        )
        r.raise_for_status()
    ...
    def cache_token(self, user):
        ...
        token1 = self.enc_aes_cbc_single("$".join(token), b"434vwt2tmqkj3trc", b"\00" * 16)
    ...
    def id_token(self, user):
        ...
        token_1 = [
            ...
            base64.encodestring(user.apk_build.encode("utf-8")),
            base64.encodestring(ms_time.encode("utf-8")),
            base64.encodestring("null".encode("utf-8")),
            base64.encodestring(str(user.device_id).encode("utf-8")),
        ]
        token = [
            md5(ms_time.encode("utf-8")).hexdigest().encode("utf-8"),
            base64.encodestring(user.apk_name.encode("utf-8")),
            base64.encodestring(user.apk_cert.encode("utf-8")),
            base64.encodestring(user.device_name.encode("utf-8")),
    ...
    def resolve_stream(self, stream):
        ...
        if stream.token == 0:
            ...
            key = "nck" + self.user.user_id[1:6] + self.user_agent[-8:]
            iv = self.user_agent[-8:] + "sdd" + self.user.user_id[1:6]
            ...
            key = "kaf" + self.user.user_id[1:6] + self.user_agent[-8:]
            iv = self.user_agent[-8:] + "ios" + self.user.user_id[1:6]
            ...
            data = {"stream_url": stream.url, "token": "34", "response_body": r.text}
            ...
            key = "ijk" + self.user.user_id[1:6] + self.user_agent[-8:]
            iv = self.user_agent[-8:] + "kms" + self.user.user_id[1:6]
            ...
            key = "akl" + self.user.user_id[1:6] + self.user_agent[-8:]
            iv = self.user_agent[-8:] + "pks" + self.user.user_id[1:6]
            ...
            headers = OrderedDict(
                [
                    ("Public-Key-Pins", b64encode("{0}.{1}".format(self.user.user_id, stream_id).encode("utf-8")),),
                    ("Modified", modified_header()),
            ...
            key = self.user.user_id[1:7] + "jib" + self.user_agent[-7:]
            iv = self.user_agent[-8:] + "sig" + self.user.user_id[1:6]
