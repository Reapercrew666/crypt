...
def Search_loop():
	...
	if Search_name == '':
		...
		HTML60 = Open_Url('https://raw.githubusercontent.com/tombebbs1/magicdragon/master/TorrentsHD2.txt')
		match2 = re.compile('<title>(.+?)</title>.+?<link>(.+?)</link>.+?<thumbnail>(.+?)</thumbnail>.+?<fanart>(.+?)/fanart>',re.DOTALL).findall(HTML60)
		for name2,url2,image2,fanart2 in match2:
			if fanart2 == '<':
				fanart2 = FANART
			else:
			...
			if 'sublink' in url2:
				...
				if Search_title in name2.lower().replace(' ',''):
					addDir(name2,url2,1,image2,fanart2,'','','','')
		HTML61 = Open_Url('https://raw.githubusercontent.com/tombebbs1/magicdragon/master/TorrentsHD3.txt')
		match2 = re.compile('<title>(.+?)</title>.+?<link>(.+?)</link>.+?<thumbnail>(.+?)</thumbnail>.+?<fanart>(.+?)/fanart>',re.DOTALL).findall(HTML61)
		for name2,url2,image2,fanart2 in match2:
			if fanart2 == '<':
				fanart2 = FANART
			else:
...
def catchuptv():
	url = 'https://123movieshub.123movies.online/?genre=drama&tv'
