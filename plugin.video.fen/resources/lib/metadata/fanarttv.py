...
def get(query, language, remote_id):
	...
	return cache_function(_get_fanart, string, (query, language, remote_id), 720, json=False)

def add(query, language, remote_id, meta):
	try:
		fanart_data = get(query, language, remote_id)
