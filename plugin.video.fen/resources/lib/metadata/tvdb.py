...
from modules.settings_reader import set_setting
...
class TvdbAPI:
	...
	def pause_token_renew(self):
		if not window.getProperty('meta.fetching.tvdb.jwtoken') == 'true':
			return False
		for i in range(0, 20):
			if window.getProperty('meta.fetching.tvdb.jwtoken') == 'true':
				time.sleep(1)
			else:
				self.jw_token = window.getProperty('meta.new.tvdb.jwtoken')
				return True

	def pause_token_new(self):
		if not window.getProperty('meta.fetching.new.tvdb.jwtoken') == 'true':
			return False
		for i in range(0, 20):
			if window.getProperty('meta.fetching.new.tvdb.jwtoken') == 'true':
				time.sleep(1)
			else:
				self.jw_token = window.getProperty('meta.new.tvdb.jwtoken')
				return True
	...
	def get_new_jwtoken(self):
		...
		if not pause_token_new:
			window.setProperty('meta.fetching.new.tvdb.jwtoken', 'true')
			window.clearProperty('meta.new.tvdb.jwtoken')
		else: return
		...
		window.setProperty('meta.new.tvdb.jwtoken', self.jw_token)
		window.clearProperty('meta.fetching.new.tvdb.jwtoken')

	def renew_jwtoken(self):
		pause_token_renew = self.pause_token_renew()
		if not pause_token_renew:
			window.setProperty('meta.fetching.tvdb.jwtoken', 'true')
			window.clearProperty('meta.new.tvdb.jwtoken')
		else: return
		...
		if 'Error' in response:
			window.clearProperty('meta.fetching.tvdb.jwtoken')
