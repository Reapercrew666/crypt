...
class Discover:
	...
	def include_genres(self):
		...
		if self._action(key) in ('clear', None): return
		if self.discover_params['db_type'] == 'movie':
			from modules.meta_lists import movie_genres as genres
		else:
			from modules.meta_lists import tvshow_genres as genres
	...
	def certification(self):
		key = 'certification'
		if self._action(key) in ('clear', None): return
		from modules.meta_lists import movie_certifications as certifications
