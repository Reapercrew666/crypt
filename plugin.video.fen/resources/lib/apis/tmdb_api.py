...
def tmdb_movie_tv_aliases(db_type, tmdb_id):
	...
	url = 'https://api.themoviedb.org/3/%s/%s/alternative_titles?api_key=%s&language=en-US' % (db_type, tmdb_id, tmdb_api)
...
def tmdb_media_images(db_type, tmdb_id):
	...
	url = 'https://api.themoviedb.org/3/%s/%s/images?api_key=%s&language=en' % (db_type, tmdb_id, tmdb_api)
	return cache_object(get_tmdb, string, url, expiration=168)
...
def tmdb_movies_crew(person_id):
	...
	url = 'https://api.themoviedb.org/3/person/%s/movie_credits?api_key=%s&language=en-US' % (int(person_id), tmdb_api)
...
def tmdb_people_tagged_pictures(actor_id, page_no):
	...
	url = 'https://api.themoviedb.org/3/person/%s/tagged_images?api_key=%s&language=en-US&page=%s' % (actor_id, tmdb_api, page_no)
	return cache_object(get_tmdb, string, url)
...
def tmdb_people_externalIDs(actor_id):
	...
	url = "https://api.themoviedb.org/3/person/%s/external_ids?api_key=%s&language=en-US" % (actor_id, tmdb_api)
	return cache_object(get_tmdb, string, url)
