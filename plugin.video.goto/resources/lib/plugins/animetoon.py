    ...
    if not xml:
        ...
        try:
            ...
            if len(pagination) > 0:
                ...
                xml += "<dir>"\
                       "    <title>[COLORgold]Next Page >>[/COLOR]</title>"\
                       "    <wctoon>%s</wctoon>"\
                       "    <thumbnail>%s</thumbnail>"\
...
def refreshtitle(title):
    ...
    title = replaceHTMLCodes(title).replace('English Dubbed','[COLOR yellow](English Dubbed)[/COLOR]').replace('English Subbed','[COLOR gold](English Subbed)[/COLOR]')
    return title
